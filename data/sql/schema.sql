...
CREATE TABLE etiqueta (id BIGINT AUTO_INCREMENT, nombre VARCHAR(255), cliente_id VARCHAR(255), contacto_id VARCHAR(255), created_at DATETIME, updated_at DATETIME, INDEX cliente_id_idx (cliente_id), INDEX contacto_id_idx (contacto_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE plantilla (id BIGINT AUTO_INCREMENT, nombre VARCHAR(255), parametros VARCHAR(255), created_at DATETIME, updated_at DATETIME, PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE etiqueta ADD CONSTRAINT etiqueta_contacto_id_contacto_id FOREIGN KEY (contacto_id) REFERENCES contacto(id);
ALTER TABLE etiqueta ADD CONSTRAINT etiqueta_cliente_id_cliente_id FOREIGN KEY (cliente_id) REFERENCES cliente(id);
